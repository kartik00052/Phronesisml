...
- **REST API decommissioned** — PhronesisML is now an **SDK-first and CLI-first offline
  Machine Learning Engineering SDK**. The entire REST subsystem has been removed:
  - `phronesisml/interfaces/api/` (FastAPI app, routes, schemas, job store) deleted.
  - `docs/guides/rest-api.md` deleted; REST sections removed from README, docs, and
    project docs.
  - `[api]` extra (`fastapi`, `uvicorn`, `python-multipart`) removed from
    `pyproject.toml`; `httpx` (dev) removed.
  - REST-only tests removed (`test_rest_*`, BUG-03 job-store, API-level tests).
  - Docker deployment for the REST server (`Dockerfile`, `docker-compose.yml`,
    `.dockerignore`) and the CI docker jobs removed.
- The Python SDK (`simple`, OOP `Phronesis`, `run_pipeline`) and the CLI
...
- **Friendly ImportError messages** — CLI (`pip install phronesisml[cli]`) modules now raise clear errors when extras are missing.
...
- **Simple API** — Zero-friction one-liner functions (`analyze()`, `train()`, `clean()`, `validate()`, `detect_target()`, `engineer()`, `select_model()`, `explain()`, `report()`) with `async` variants for Jupyter async contexts. Each returns a frozen dataclass with documented fields.
...
- **Async loop guard** — `_ensure_sync()` raises `RuntimeError` with a clear message if called from inside a running event loop (Jupyter async), preventing silent hangs.
...
- **FastAPI interface** *(removed in v0.3.0)* — REST endpoints with multipart file upload, background job execution, job status polling, and OpenAPI docs (`pip install aetherml[api]`).
...
- **CI/CD pipeline** — GitHub Actions workflow with lint (ruff), typecheck (mypy), tests (pytest across Python 3.11/3.12/3.13), CLI tests, explainability tests, Docker build, and GHCR image publishing on tagged releases. *(API tests and Docker jobs removed in v0.3.0.)*
- **Docker image** *(removed in v0.3.0)* — Multi-stage Dockerfile producing a minimal production image serving the REST API, published to `ghcr.io/kartik00052/aetherml`.
- **Core dependencies required by default** — pandas, polars, numpy, scikit-learn, pydantic, langgraph, and joblib are always installed. Optional extras were: `[api]`, `[cli]`, `[spark]`, `[explain]`, `[boost]`, `[mlflow]`. *(The `[api]` extra was removed in v0.3.0.)*
- **`python-multipart` in API extras** *(removed in v0.3.0)* — Required by FastAPI for file upload parsing; added to the `[api]` extra.

### Changed

- **SDK-first architecture** — CLI is now a thin client that delegates entirely to the SDK; no business logic lives outside the SDK.
...
- **Mypy strictness** — Enabled `strict = true` with `python_version = "3.13"` and per-module overrides for Typer decorator types.
...
- **CI test isolation** — `conftest.py` used `collect_ignore` to skip `test_api.py` and `test_cli_app.py` when FastAPI/Typer were not installed. *(The API test skip was removed in v0.3.0.)*
