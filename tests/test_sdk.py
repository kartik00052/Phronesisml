"""Comprehensive tests for the Phronesis public SDK (beginner API)."""
...
@pytest.fixture
def ml(csv_path: str) -> Phronesis:
    """Create an Phronesis instance with a CSV dataset."""
    return Phronesis(csv_path)
...
class TestImports:
    ...
    def test_import_performance(self) -> None:
        """Importing Phronesis should be fast (< 2s)."""
        start = time.monotonic()
        import importlib

        import phronesisml

        importlib.reload(phronesisml)
    ...
    def test_backward_compatible_imports(self) -> None:
        ...
        from phronesisml import PhronesisConfig, WorkflowState, run_pipeline  # noqa: F811

        assert run_pipeline is not None
        assert PhronesisConfig is not None
...
class TestConstructor:
    ...
    def test_default_config(self, csv_path: str) -> None:
        ml = Phronesis(csv_path)
        assert isinstance(ml.config, PhronesisConfig)
...
class TestReport:
    def test_returns_markdown_string(self, csv_path: str) -> None:
        ...
        assert len(r) > 100
        assert "# Phronesis Pipeline Report" in r
...
class TestErrorHandling:
    def test_missing_file_raises(self) -> None:
        ...
        ml = Phronesis("/nonexistent/file.csv")
...
class TestBackwardCompatibility:
    ...
    def test_advanced_api_unchanged(self) -> None:
        ...
        from phronesisml import PhronesisConfig, WorkflowState  # noqa: F811
