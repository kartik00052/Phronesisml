...
existing ``AetherML`` class (which itself delegates to LangGraph-orchestrated
...
def _build_config(
    *,
    ...
    """Build an AetherMLConfig from flat keyword arguments."""
...
def _build_dataset_profile(ml: Any) -> DatasetProfile:
    """Build DatasetProfile from AetherML state after EDA."""
    import pandas as pd
...
def _build_clean_result(ml: Any) -> CleanResult:
    """Build CleanResult from AetherML state after ETL."""
    import pandas as pd
...
def _build_validation_result(ml: Any) -> ValidationResult:
    """Build ValidationResult from AetherML state after validation."""
    import pandas as pd
...
def _build_target_result(ml: Any) -> TargetResult:
    """Build TargetResult from AetherML state after target detection."""
    state = ml._state
    return TargetResult(
...
def _build_feature_result(ml: Any) -> FeatureResult:
    """Build FeatureResult from AetherML state after feature engineering."""
    state = ml._state
...
def _build_model_result(ml: Any) -> ModelResult:
    """Build ModelResult from AetherML state after evaluation."""
    state = ml._state
...
def _build_explain_result(ml: Any) -> ExplainResult:
    """Build ExplainResult from AetherML state after explainability."""
    state = ml._state
...
def _build_train_result(ml: Any) -> TrainResult:
    """Build TrainResult from AetherML state after full pipeline."""
    state = ml._state
    bp = state.best_pipeline or {}
    er = state.explanation_report or {}
    return TrainResult(
...
def detect_target(
    ...
    *,
    ...
    Example::
        ...
        from aetherml import detect_target
...
def select_model(
    ...
    *,
    ...
    Example::
        ...
        from aetherml import select_model
