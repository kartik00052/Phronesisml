...
def _row(name: str, *, user_id: str | None = None, kind: str = "run") -> dict:
...
def test_dataset_hidden_from_foreign_user() -> None:
    d = repositories.create_dataset(_row("foreign-ds", user_id="user-a", kind="ds"))
    assert repositories.get_dataset(d.id, user_id="user-b") is None


def test_shared_dataset_visible_to_everyone() -> None:
    d = repositories.create_dataset(_row("shared-sample", user_id=None, kind="ds"))
...
def test_user_a_cannot_delete_user_b_dataset(client: TestClient) -> None:
    d = repositories.create_dataset(_row("theirs", user_id="user-a", kind="ds"))
