...
def test_run_ownership_isolation(repositories_created=False):
    ...
    run_a = repositories.create_run({"name": "owner-visible"}, user_id="user-a")
...
def test_dataset_ownership_isolation():
    ds_a = repositories.create_dataset({"name": "owner-ds", "format": "csv"}, user_id="user-a")
...
def test_require_owned_run_hides_foreign_run():
    ...
    run_a = repositories.create_run({"name": "theirs"}, user_id="user-a")
...
def test_require_owned_run_allows_owner():
    run_a = repositories.create_run({"name": "mine"}, user_id="user-a")
    require_owned_run(run_a["id"], "user-a")  # no raise


def test_require_owned_dataset_hides_foreign_dataset():
    ds_a = repositories.create_dataset({"name": "ds-a", "format": "csv"}, user_id="user-a")
    with pytest.raises(HTTPException) as exc:
